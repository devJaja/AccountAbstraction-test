...
contract AbstractAccount is IAccount, Ownable {
    ...
    error AbstractAccount__PaymasterValidationFailed();

    IEntryPoint private immutable i_entryPoint;
    IPaymaster private immutable i_paymaster;
    ...
    constructor(address entryPoint) Ownable(msg.sender) {
        ...
        i_paymaster = IPaymaster(paymaster); 
    }
    ...
    function validateUserOp(
        ...
        (bytes memory context, uint256 paymasterValidationData) = i_paymaster.validatePaymasterUserOp(
            userOp,
            userOpHash,
            missingAccountFunds
        );
        ...
        if (paymasterValidationData != 0) {
            revert AbstractAccount__PaymasterValidationFailed();
        }
